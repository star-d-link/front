...
function KakaoMapEdit({ initialLat, initialLng, initialAddress, onLocationSelect }) {
  ...
  useEffect(() => {
    // 초기 좌표가 주어질 경우 해당 좌표에 마커를 하나 표시
    if (!map || initialLat == null || initialLng == null) return;

    const initialMarker = {
      position: { lat: initialLat, lng: initialLng },
      content: initialAddress || "초기 위치",
  ...
  }, [map, initialLat, initialLng, initialAddress]);
  ...
  const EventMarkerContainer = ({ position, content, roadAddress, address, i }) => {
    ...
    // 선택된 마커만 보여주고, 검색 결과가 있을 경우에는 검색 결과와 선택 비교
    // 여기서는 KakaoKeywordMap와는 다르게, 초기 마커가 있을 경우에도
    // 검색 결과 마커를 보여줄 때는 selectedMarkerIndex를 기준으로 처리한다.
    if (selectedMarkerIndex !== null && selectedMarkerIndex !== i && keyword) {
      return null;
    }
  ...
  return (
      <div className="map_wrap">
        <Map
            center={{
              lat: initialLat || 37.566826,
              lng: initialLng || 126.9786567,
            }}
            style={{
              width: "100%",
              height: "500px",
